...
pub async fn connect(conn: &ResolvedConnection) -> AppResult<Arc<dyn Integration>> {
    ...
    let mut builder = ClientBuilder::new(servers.clone())
        .client_id("db-free")
    ...
    if let Some(user) = user {
        builder = builder.sasl_config(SaslConfig::Plain(Credentials::new(
            user.to_string(),
            conn.secret.clone().unwrap_or_default(),
        )));
    }
...
pub fn parse_command(text: &str, max_rows: usize) -> AppResult<Command> {
    ...
    if text.starts_with('{') {
        ...
        if let Some(produce) = obj.get("produce") {
            let p = produce
                .as_object()
                .ok_or_else(|| AppError::invalid_input("\"produce\" must be an object."))?;
            let topic = p
                .get("topic")
                .map(json_string)
                .filter(|t| !t.is_empty())
                .ok_or_else(|| AppError::invalid_input("\"produce.topic\" is required."))?;
            let value = p
                .get("value")
                .map(json_string)
                .ok_or_else(|| AppError::invalid_input("\"produce.value\" is required."))?;
        ...
        let topic = obj
            ...
            .map(json_string)
            .filter(|t| !t.is_empty())
            .ok_or_else(|| AppError::invalid_input("\"topic\" is required."))?;
        let partition = obj
            .get("partition")
            .and_then(serde_json::Value::as_i64)
            .and_then(|v| i32::try_from(v).ok());
        let start = match obj.get("offset") {
            ...
            Some(serde_json::Value::String(s)) if s.eq_ignore_ascii_case("earliest") => {
                Start::Earliest
            }
            Some(serde_json::Value::String(s)) if s.eq_ignore_ascii_case("latest") => Start::Latest,
            Some(serde_json::Value::String(s)) => Start::At(s.parse().map_err(|_| {
                AppError::invalid_input("\"offset\" must be a number, \"earliest\" or \"latest\".")
            })?),
            Some(_) => {
                return Err(AppError::invalid_input(
                    "\"offset\" must be a number, \"earliest\" or \"latest\".",
                ))
            }
        };
        let limit = obj
...
impl KafkaIntegration {
    ...
    async fn partitions_of(&self, topic: &str) -> AppResult<Vec<i32>> {
        ...
        let found = topics
            .into_iter()
            .find(|t| t.name == topic)
            .ok_or_else(|| AppError::not_found(format!("Topic \"{topic}\" not found.")))?;
    ...
    async fn read_range(
        ...
        take: u64,
        high: i64,
    ) -> AppResult<Vec<RecordAndOffset>> {
        let mut out = Vec::new();
        let mut cursor = start;
        while (out.len() as u64) < take && cursor < high {
            let (records, _) = pc
                .fetch_records(cursor, 1..FETCH_BYTES, FETCH_WAIT_MS)
                .await
                .map_err(map_error)?;
            if records.is_empty() {
                break;
    ...
    async fn produce(
        ...
        if self.read_only {
            return Err(AppError::invalid_input(
                "This connection is read-only; producing is blocked.",
            ));
        }
        ...
        pc.produce(vec![record], Compression::default())
            .await
            .map_err(map_error)?;
...
impl PartitionOffsets {
    ...
    fn row(&self) -> Vec<Value> {
        vec![
            Value::Int(i64::from(self.partition)),
            Value::Int(self.low),
            Value::Int(self.high),
            Value::Int(self.records()),
        ]
    }
...
pub(crate) fn offsets_result(offsets: &[PartitionOffsets]) -> ResultSet {
    ResultSet {
        ...
        rows: offsets.iter().map(PartitionOffsets::row).collect(),
        truncated: false,
    }
}
...
impl KafkaIntegration {
    async fn delete_topic(&self, topic: &str) -> AppResult<StatementResult> {
        if self.read_only {
            return Err(AppError::read_only(
                "This connection is read-only; deleting a topic is blocked.",
            ));
        }
        let controller = self.client.controller_client().map_err(map_error)?;
        controller
            .delete_topic(topic, DELETE_TIMEOUT_MS)
            .await
            .map_err(map_error)?;
        Ok(StatementResult::Affected { rows_affected: 1 })
    }
    ...
    async fn topic_detail(&self, reference: &ObjectRef) -> AppResult<ObjectDetail> {
        ...
        let mut detail = ObjectDetail::empty(reference)
            .definition(
                format!("{{\"topic\": \"{name}\", \"offset\": \"earliest\", \"limit\": 100}}"),
                CodeLanguage::Json,
            )
        ...
        if let (Some(first), Some(last)) = (
            offsets.iter().map(|o| o.low).min(),
            offsets.iter().map(|o| o.high).max(),
        ) {
    ...
    async fn partition_detail(&self, reference: &ObjectRef) -> AppResult<ObjectDetail> {
        let topic = reference
            .parent
            .clone()
            .ok_or_else(|| AppError::invalid_input("A partition needs its topic."))?;
        let id: i32 = reference.name.parse().map_err(|_| {
            AppError::invalid_input(format!("`{}` is not a partition number.", reference.name))
        })?;
        let pc = self.partition(&topic, id).await?;
        let (low, high) = self.watermarks(&pc).await?;
        let o = PartitionOffsets {
            partition: id,
            low,
            high,
        };
    ...
    async fn stats(&self) -> AppResult<ServerStats> {
        ...
        for t in &topics {
            ...
            let n: i64 = self
                .partition_offsets(&t.name)
                .await
                .unwrap_or_default()
                .iter()
                .map(PartitionOffsets::records)
                .sum();
            records += n;
            if n == 0 {
                empty += 1;
        ...
        let cluster = vec![
            Stat::text(
                "Engine",
                if self.engine == Engine::Redpanda {
                    "Redpanda"
                } else {
                    "Kafka"
                },
            ),
            Stat::number("Bootstrap brokers", self.servers.len() as f64, None)
                .with_hint(self.servers.join(", ")),
        ...
        let mut throughput = vec![Stat::number("Records retained", records as f64, None)
            .with_hint("Σ (high − low watermark) over non-internal topics")];
...
impl Integration for KafkaIntegration {
    ...
    async fn catalog(&self) -> AppResult<SchemaCatalog> {
        ...
        for t in topics {
            ...
            tables.push(TableInfo {
                schema: Some(TOPICS_SCHEMA.to_string()),
                name: t.name,
    ...
    async fn fetch_page(&self, table: &TableRef, query: &PageQuery) -> AppResult<ResultSet> {
        ...
        let names: Vec<String> = COLUMN_NAMES.iter().map(|n| (*n).to_string()).collect();
        let needs_local =
            !query.filters.is_empty() || query.sort.iter().any(|s| s.column != "offset" || !s.desc);
        let wanted = query
            .offset
        ...
        let (rows, local_query) = if needs_local {
            ...
            (
                self.tail(&table.name, &partitions, MAX_RECORDS, 0).await?,
                query.clone(),
            )
        } else {
        ...
        if local_query.sort.is_empty() {
            local_query.sort = vec![crate::model::SortRule {
                column: "offset".into(),
                desc: true,
            }];
        }
    ...
    async fn execute(&self, script: &str, max_rows: usize) -> AppResult<Vec<StatementResult>> {
        ...
        for statement in statements {
            let result = match parse_command(statement, max_rows)? {
                Command::Topics => StatementResult::Rows {
                    result: self.topics_result().await?,
                },
                Command::Consume {
                    topic,
                    partition,
                    ...
                    result: self.consume(&topic, partition, &start, limit).await?,
                },
                Command::Produce {
                    topic,
                    partition,
                    ...
                    value,
                    headers,
                } => self.produce(&topic, partition, key, value, headers).await?,
...
// WHY:   Shared with the RabbitMQ and Convex consoles, which accept the same mix.
...
mod tests {
    use super::*;
    use crate::model::{
        ConnectionInput, ConnectionSummary, Environment, FilterOp, SortRule, SslMode,
    };

    #[test]
    fn bootstrap_list_gets_ports() {
        assert_eq!(
            bootstrap_servers(Some("a:1, b ,c:3"), Some(9092)),
            vec!["a:1", "b:9092", "c:3"]
        );
    ...
    fn commands_parse() {
        assert_eq!(
            parse_command("TOPICS", 10).unwrap_or(Command::Topics),
            Command::Topics
        );
        assert_eq!(
        ...
        assert_eq!(
            parse_command(
                r#"{"topic": "t", "partition": 2, "offset": "earliest", "limit": 5000}"#,
                100
            )
            .ok(),
            Some(Command::Consume {
        ...
        let produce = parse_command(
            r#"{"produce": {"topic": "t", "key": 1, "value": {"a": 1}, "headers": {"h": "v"}}}"#,
            10,
        )
        .ok();
        assert_eq!(
        ...
        assert!(matches!(
            parse_command("SELECT 1", 10),
            Err(AppError::InvalidInput { .. })
        ));
        assert!(matches!(
            parse_command(r#"{"offset": 1}"#, 10),
            Err(AppError::InvalidInput { .. })
        ));
        assert!(matches!(
            parse_command(r#"{"topic": "t", "offset": true}"#, 10),
            Err(AppError::InvalidInput { .. })
        ));
    }
    ...
    fn delete_topic_parses_and_offsets_shape_rows() {
        assert_eq!(
            parse_command(r#"{"delete": {"topic": "orders"}}"#, 10).ok(),
            Some(Command::DeleteTopic {
                topic: "orders".into()
            })
        );
        assert_eq!(
            parse_command(r#"{"delete": "orders"}"#, 10).ok(),
            Some(Command::DeleteTopic {
                topic: "orders".into()
            })
        );
        assert!(matches!(
            parse_command(r#"{"delete": {}}"#, 10),
            Err(AppError::InvalidInput { .. })
        ));
        ...
        assert_eq!(
            rs.columns
                ...
                .map(|c| c.name.as_str())
                .collect::<Vec<_>>(),
            vec!["partition", "low_watermark", "high_watermark", "records"]
        );
        assert_eq!(
            rs.rows[0],
            vec![Value::Int(0), Value::Int(5), Value::Int(12), Value::Int(7)]
        );
        ...
        assert_eq!(
            KafkaIntegration::topic_summary("orders", 1, None)
                .detail
                .as_deref(),
            Some("1 partitions")
        );
    ...
    fn resolved(engine: Engine, host: String) -> ResolvedConnection {
        let input = ConnectionInput {
            ...
            read_only: false,
            host: Some(host),
            port: std::env::var("DBFREE_TEST_KAFKA_PORT")
                .ok()
        ...
        ResolvedConnection {
            ...
            secret: std::env::var("DBFREE_TEST_KAFKA_PASSWORD").ok(),
        }
    }
    ...
    async fn live_round_trip_when_configured() {
        ...
        let kafka = connect(&resolved(Engine::Kafka, host))
            .await
        ...
        if controller {
            ...
            if let Ok(cc) = kafka_client_for_test(&resolved(
                Engine::Kafka,
                std::env::var("DBFREE_TEST_KAFKA_HOST").unwrap_or_default(),
            ))
            .await
            {
        ...
        for i in 0..3 {
            ...
            kafka
                .execute(&cmd, 10)
                .await
                .unwrap_or_else(|e| panic!("produce: {e}"));
        }
        let catalog = kafka
            .catalog()
            .await
            .unwrap_or_else(|e| panic!("catalog: {e}"));
        let table = catalog.schemas[0]
            .tables
            .iter()
            .find(|t| t.name == topic)
            .unwrap_or_else(|| panic!("topic missing: {catalog:?}"));
        assert!(table.row_estimate.unwrap_or_default() >= 3);
        let table = TableRef {
            schema: Some("topics".into()),
            name: topic.into(),
        };
        let page = kafka
            .fetch_page(
        ...
        let out = kafka
            .execute(&format!("CONSUME {topic} 2"), 10)
            .await
        ...
        let topics = kafka
            .execute("TOPICS", 10)
            .await
            .unwrap_or_else(|e| panic!("topics: {e}"));
        assert!(
            matches!(&topics[0], StatementResult::Rows { result } if result.rows.iter().any(|r| r[0] == Value::Text(topic.into())))
        );
    }
