// SOT: cloudflare-d1-integration, d1-adapter, cloudflare-rest-api, sqlite-over-http, d1-object-explorer
...
impl CloudflareD1Integration {
    ...
    // WHAT:  One catalog query as name → value maps (D1 answers with JSON objects).
    async fn catalog_rows(&self, sql: &str) -> AppResult<Vec<CatalogRow>> {
        let block = self.run_sql(sql).await?;
        Ok(block
            .results
            .into_iter()
            .map(|row| row.iter().map(|(name, value)| (name.clone(), json_to_value(value))).collect())
            .collect())
    }

    // WHAT:  Catalog rows, or none when D1 rejects the query (PRAGMA support varies).
...
// OBJECT EXPLORER
//
// WHAT:  Tables, views, indexes and triggers of a D1 database, read through the
//        same /query REST endpoint as every other statement.
// WHY:   D1 is SQLite on Cloudflare's edge: sqlite_master and the PRAGMA
//        statements are the whole catalog it exposes.
// HOW:   D1 already answers with name-keyed JSON objects, so a row is a
//        `CatalogRow` map and the builders below are pure and testable offline.
//        D1 only whitelists some PRAGMAs: every PRAGMA read may fail and the
//        explorer returns what worked.
// WHERE: src-tauri/src/model/objects.rs, src/features/objects/ObjectTab.tsx
// ============================================================================

const OBJECT_CAP: usize = 2000;

// WHAT:  One catalog row keyed by column name (D1's own JSON shape).
...
// HOW:   D1 also hides its own bookkeeping tables behind the `_cf_` prefix.
fn master_query(kind: &str, table: Option<&str>) -> String {
    ...
    format!(
        "SELECT type, name, tbl_name, sql FROM sqlite_master \
         WHERE type = '{kind}' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE '\\_cf\\_%' ESCAPE '\\'{owner} \
         ORDER BY name LIMIT {OBJECT_CAP}"
    )
}
...
mod tests {
    ...
    #[test]
    fn catalog_queries_hide_internal_objects() {
        let sql = master_query("table", None);
        assert!(sql.contains("name NOT LIKE 'sqlite_%'"));
        assert!(sql.contains("name NOT LIKE '\\_cf\\_%' ESCAPE '\\'"), "{sql}");
        assert!(sql.ends_with("ORDER BY name LIMIT 2000"));
    ...
    fn master_rows_become_summaries() {
        ...
        assert_eq!(master_summary(ObjectKind::Table, &table).and_then(|s| s.reference.parent).as_deref(), Some("main"));
        ...
        let index = row(&[("name", text("i")), ("tbl_name", text("users")), ("sql", text("CREATE INDEX i ON users(a)"))]);
        ...
        assert_eq!(summary.badge.as_deref(), Some("index"));

        let trigger = row(&[("name", text("t")), ("tbl_name", text("orders")), ("sql", text("CREATE TRIGGER t AFTER INSERT ON orders BEGIN SELECT 1; END"))]);
        assert_eq!(master_summary(ObjectKind::Trigger, &trigger).and_then(|s| s.badge).as_deref(), Some("insert"));
        assert!(master_summary(ObjectKind::Database, &trigger).is_none());
    }
    ...
    fn pragma_rows_become_columns_and_grids() {
        ...
        assert!(columns[1].nullable && columns[1].ordinal == 1);

        let grid = to_result_set(&rows, &["name", "pk", "missing"]);
        assert_eq!(grid.columns.len(), 3);
        assert_eq!(grid.rows[0][0], text("id"));
        assert_eq!(grid.rows[0][2], Value::Null);
        assert_eq!(cell_i64(&row(&[("n", text("5"))]), "n"), Some(5));
        assert_eq!(cell_text(&row(&[("n", Value::Null)]), "n"), None);
        assert_eq!(virtual_module("CREATE VIRTUAL TABLE t USING fts5(a)").as_deref(), Some("fts5"));
        assert_eq!(index_badge(None), "auto");
        assert_eq!(trigger_facts("CREATE TRIGGER t INSTEAD OF UPDATE ON v BEGIN SELECT 1; END"), (Some("INSTEAD OF"), Some("UPDATE")));
    }
}
