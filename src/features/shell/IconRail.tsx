      ...
      <div className="flex flex-col items-center gap-1.5">
        ...
        {SIDEBARS.map((s) => (
          <IconButton
            ...
            disabled={!connected}
            onClick={() => setSidebar(s.mode)}
            size={16}
          />
        ))}
        <Separator className="my-1.5 w-5 opacity-40" />
        <IconButton icon="history" label="Query history" disabled={!connected} onClick={() => activeId && openHistory(activeId)} size={16} />
        <IconButton icon="download" label="Export / Import" disabled={!connected} onClick={() => activeId && openTransfer(activeId)} size={16} />
        <IconButton icon="braces" label="Chat with database" disabled={!connected} onClick={() => activeId && openChat(activeId)} size={16} />
        <IconButton icon="server" label="Server admin" disabled={!connected} onClick={() => activeId && openAdmin(activeId)} size={16} />
      </div>
      <div className="mt-auto flex flex-col items-center gap-1">
        <IconButton icon="grid" label="Engine capabilities" active={page.kind === "capabilities"} onClick={goCapabilities} size={16} />
        <IconButton icon="settings" label="Settings" active={page.kind === "settings"} onClick={goSettings} size={16} />
      </div>
    </nav>
  );
}
